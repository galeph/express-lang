...
module.exports = function (settings) {
	...
	settings.store.__set('session', settings.session);
	...
	return function local (req, res, next) {
		settings.store.setLang(req, function (err, lang, code){
			...
			res[ settings.localte ] = res.locals[ settings.localte ] = settings.store.translate(lang);

			if(req.session && code )
				req.session[ settings.session ] = code;
			
			if( !settings.store.listLang )
				return next();
